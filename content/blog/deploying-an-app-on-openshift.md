...
First, a little about the app we'll be deploying. This is a very basic Todo list app that can add, update, and delete todo items from a list. Since this post is mostly focused on the simplicity of deploying an app on OpenShift, I tried to keep the application code as minimal and readable as possible. The frontend is written in [Svelte](https://svelte.dev/), a Javascript framework known for it's speed and simplicity. This is my first attempt at a Svelte app, so be kind. 

The Todo API is written in C#, and uses the excellent [featherhttp framework](https://github.com/featherhttp/framework) from David Fowler. I used one of David's [todo examples](https://github.com/davidfowl/Todos) and added EF support for SQL server, some minimal CORS settings and a few additional methods. 
...
The code for both the web app and API is available in my [GitHub todo repo](https://github.com/michaelburch/todo). 
...
{% <caption_image src="/images/openshift-deploy-image.png" alt="" style="container"> %}1. Select image{% </caption_image> %}

</td>
<td v-align="middle" align="center">

{% <caption_image src="/images/openshift-deploy-image-2.png" alt="" style="container"> %}2. Provide names and click create{% </caption_image> %}

</td>
</tr>
</table>

> Note that I'm using a public Docker image from the Microsoft Container Registry, mcr.microsoft.com/mssql/server:2019-latest. You can browse available images and find required variables and usage instructions on [Docker hub](https://hub.docker.com/_/microsoft-mssql-server)
...
{% <caption_image src="/images/openshift-edit-deploy.png" alt="" style="container"> %}3. Right-click, select Edit Deployment{% </caption_image> %}

</td>
<td v-align="middle" align="center">

{% <caption_image src="/images/openshift-edit-deploy-2.png" alt="" style="container"> %}4. Select Environment,set values, click save{% </caption_image> %}
...
First, I'll click '_Add_' and select the '_From Git_' option. On the following screen I'll provide my Git repo URL, [https://github.com/michaelburch/todo](https://github.com/michaelburch/todo), and since I have both the web app and API in a single repo I will use advanced options to specify a path so that only the API is built.  
...
{% <caption_image src="/images/openshift-add-git.png" alt="" style="container"> %}1. Add from Git{% </caption_image> %}

</td>
<td v-align="middle" align="center">

{% <caption_image src="/images/openshift-config-repo.png" alt="" style="container"> %}2. Provide repository details{% </caption_image> %}
...
{% <caption_image src="/images/openshift-select-builder.png" alt="" style="container"> %}3. Select builder image{% </caption_image> %}

</td>
<td v-align="middle" align="center">

{% <caption_image src="/images/openshift-name-api.png" alt="" style="container"> %}4. Name and create{% </caption_image> %}
...
{% <caption_image src="/images/openshift-edit-api.png" alt="" style="container"> %}5. Right-click, select Edit DeploymentConfig{% </caption_image> %}

</td>
<td v-align="middle" align="center">

{% <caption_image src="/images/openshift-edit-api-2.png" alt="" style="container"> %}6. Select Environment,set values, click save{% </caption_image> %}
...
{% <caption_image src="/images/openshift-edit-buildconfig.png" alt="" style="container"> %}1. Click build configuration under advanced options{% </caption_image> %}

</td>
<td v-align="middle" align="center">

{% <caption_image src="/images/openshift-edit-buildconfig-2.png" alt="" style="container"> %}2. Enter values, click create{% </caption_image> %}
